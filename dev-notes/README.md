# Metallurgy Development Documentation

## Core Concepts

- [Project Structure](./core/project-structure.md)
- [.metallurgy Configuration](./core/metallurgy-config.md)

## User Flows

- [Project Selection](./flows/project-selection.md)
- [File Editing](./flows/file-editing.md)
- [Template Management](./flows/template-management.md)

## Technical Documentation

### Data Flow

- [YAML to Form Conversion](./technical/yaml-to-form.md)
- [Form to Object Transformation](./technical/form-to-object.md)
- [Storage Operations](./technical/storage-operations.md)

### Components

- [Edit Form](./components/edit-form.md)
- [Sidebar Navigation](./components/sidebar.md)
- [Field Types](./components/field-types.md)

## Implementation Details

- [Installation & Setup](./implementation/installation.md)
- [Build Process](./implementation/build-process.md)
- [Testing Strategy](./implementation/testing.md)

## Migration & Refactoring

- [Current Architecture](./refactoring/current-architecture.md)
- [Target Architecture](./refactoring/target-architecture.md)
- [Migration Steps](./refactoring/migration-steps.md)
