...
export const FileTreeBase = memo(
  ({ fileTree, fileSelected, onFileClick, fileType, FileIcon, onFolderActivate, activeFolder }) => {
    ...
    const contentRoot = fileTree
      ? Object.keys(fileTree).find((key) => StorageOperations.getContentPath()?.includes(key))
      : null;

    const dataRoot = fileTree
      ? Object.keys(fileTree).find((key) => StorageOperations.getDataPath()?.includes(key))
      : null;
        ...
        return [
          ...
          ...folders.map(([key, value]) => {
            ...
            return (
              <li
                key={key}
                className={`folder ${isOpen ? 'open' : ''} ${isActive ? 'active' : ''}`}
              >
                ...
                <ul className="folder-content">{renderTreeNode(value, currentPath)}</ul>
              </li>
            );
          })
    ...
    return (
      <div className="file-tree">
        <ul className="dom-tree">{renderTreeNode(fileTree)}</ul>
      </div>
    );
  }
);
