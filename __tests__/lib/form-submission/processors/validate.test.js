...
describe('Form Validation', () => {
  ...
  describe('validateSubmission', () => {
    ...
    test('should validate different data types correctly', () => {
      ...
      expect(validateSubmission({ count: 'not-a-number' }, { properties: { count: { type: 'number' } } })).toHaveLength(1);
      expect(validateSubmission({ count: 42 }, { properties: { count: { type: 'number' } } })).toHaveLength(0);
      
      // Test boolean validation
      expect(validateSubmission({ isActive: 'maybe' }, { properties: { isActive: { type: 'boolean' } } })).toHaveLength(1);
      expect(validateSubmission({ isActive: true }, { properties: { isActive: { type: 'boolean' } } })).toHaveLength(0);
      
      // Test date validation
      expect(validateSubmission({ date: 'not-a-date' }, { properties: { date: { type: 'date' } } })).toHaveLength(1);
      expect(validateSubmission({ date: '2023-06-15T00:00:00.000Z' }, { properties: { date: { type: 'date' } } })).toHaveLength(0);
      
      // Test array validation
      expect(validateSubmission({ items: 'not-an-array' }, { properties: { items: { type: 'array' } } })).toHaveLength(1);
      expect(validateSubmission({ items: [] }, { properties: { items: { type: 'array' } } })).toHaveLength(0);
    });
  });
});